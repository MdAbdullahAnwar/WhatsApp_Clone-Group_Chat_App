...
const initializeSocket = (server) => {
  const io = new Server(server, {
    ...
    pingTimeout: 60000, //DISCONNECT intactive users or sockets after 60s
  });
  ...
  io.on("connection", (socket) => {
    ...
    // Return online status of required user
      ...
      async ({ messageId, emoji, userId, reactionUserId }) => {
        try {
          ...
          const reactionUpdated = {
            messageId,
            reactions: populatedMesssage.reactions,
          };

          const senderSocket = onlineUsers.get(
            populatedMesssage.sender._id.toString()
          );
          const receiverSocket = onlineUsers.get(
            populatedMesssage.receiver?._id.toString()
          );
